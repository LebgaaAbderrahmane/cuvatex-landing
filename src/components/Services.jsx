...
export default function Services() {
  ...
  // rewrapping on a language switch (the Arabic h2 wraps taller), and the header
  // growing on narrow viewports — none of which a `resize` listener alone catches.
  useLayoutEffect(() => {
    const title = titleRef.current;
    if (!title) return;
    const header = document.querySelector('header');
    const measure = () => {
      setTitleH(title.getBoundingClientRect().height);
  ...
  return (
    ...
    >
      {/* Sticky scope. A sticky element only travels inside its own parent's box, so
          this wrapper is what decides when the title lets go. It is deliberately one
          card shorter than the card container, which makes its bottom edge land on
          the exact scroll position where the last card stops being pinned — so the
          browser pushes the title out in lockstep with that card scrolling away.
